...
func renderReadme() string {
	...
	b.WriteString("```bash\ngo run ./build\n```\n\n")
	b.WriteString("That reads `content/*.json` and writes the pages to the repository root, plus `sitemap.xml`, `robots.txt`, `llms.txt`, `llms-full.txt` and `BUILD-MANIFEST.md`. ")
	...
	b.WriteString("### The site is the specification\n\n")
	b.WriteString("The site describes the product we intend to sell, including parts that are designed and not yet built. ")
	b.WriteString("So it doubles as the feature spec, and `BUILD-MANIFEST.md` is the honest answer for where each part stands: ")
	b.WriteString("every page, section and feature with a status, generated from the same content the pages are built from, so the two cannot drift.\n\n")
	b.WriteString("Statuses live in `content/_status.json`, which is small and meant to be hand edited. ")
	b.WriteString("A section or feature with no entry inherits its page status, so the file only needs an entry where something differs. ")
	b.WriteString("When a feature ships, flip its status, rebuild, and the manifest and the site both update.\n\n")
	b.WriteString("## Layout\n\n")
	b.WriteString("| Path | What it is |\n|---|---|\n")
	b.WriteString("| `content/*.json` | One file per page. The source of truth for all copy |\n")
	b.WriteString("| `content/_status.json` | Build status per page, section and feature. Feeds `BUILD-MANIFEST.md` |\n")
	b.WriteString("| `BUILD-MANIFEST.md` | Generated. The spec against reality, as a checklist |\n")
	...
	b.WriteString("## How it deploys\n\n")
	b.WriteString("Committing to `main` updates alphaflux.net. Nothing is built by hand and nothing is copied to the server.\n\n")
	b.WriteString("```\n")
	b.WriteString("git push origin main\n")
	b.WriteString("  -> GitHub webhook fires on push\n")
	b.WriteString("  -> https://dash.alphaflux.net/api/deploy/<app token>\n")
	b.WriteString("  -> Dokploy clones the repo and builds the Dockerfile\n")
	b.WriteString("  -> swarm service alphaflux-web-qjojtn is updated\n")
	b.WriteString("  -> Traefik serves it at alphaflux.net\n")
	b.WriteString("```\n\n")
	b.WriteString("Settings in Dokploy that must not drift. All three have been wrong before and all three are\n")
	b.WriteString("silent when they are:\n\n")
	b.WriteString("| Setting | Value | Why |\n|---|---|---|\n")
	b.WriteString("| `buildType` | `dockerfile` | The default is nixpacks, which does not know about this Dockerfile |\n")
	b.WriteString("| `dockerfile` | `Dockerfile` | |\n")
	b.WriteString("| `buildPath` | `/` | |\n")
	b.WriteString("| `autoDeploy` | `true` | Without it the webhook is ignored |\n\n")
	b.WriteString("Routing lives outside Dokploy, in `/etc/dokploy/traefik/dynamic/alphaflux-web.yml` on the server.\n")
	b.WriteString("That file names the swarm service, and Dokploy appends a suffix to the service name it creates,\n")
	b.WriteString("so the file says `alphaflux-web-qjojtn`. **If the Dokploy app is ever deleted and recreated the\n")
	b.WriteString("suffix changes and that file has to be updated**, or the site will 404 at the edge while the\n")
	b.WriteString("container runs perfectly. The console works the same way as `alphaflux-login-xsqq9t`.\n\n")
	b.WriteString("To find the current service name: `docker service ls | grep alphaflux-web`.\n\n")
